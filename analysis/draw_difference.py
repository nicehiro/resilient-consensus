...
def draw_trajectory(path: List[str], fig_title, save_name):
    ...
    fig.tight_layout()
    fig.subplots_adjust(left=0.1, bottom=0.168, right=0.982, top=0.884, wspace=0.442, hspace=0.855)
        ...
        with plt.style.context(["science", "ieee"]):
            for i in range(8):
                l = axs[j // axs_y][j % axs_y].plot(
                    df[df.iloc[:, 0] % 10 == 0].iloc[:, i+5] - df[df.iloc[:, 0] % 10 == 0].iloc[:, i+6 if i < 7 else 5],
...
if __name__ == "__main__":
    ...
    file_path = ["q_c_switcher_rrrr_9.csv", "q_c_switcher_rrcc_9.csv", "q_c_switcher_cccc_9.csv"] + \
                ["q_c_switcher_rrrr_6.csv", "q_c_switcher_rrcc_6.csv", "q_c_switcher_cccc_6.csv"] + \
                ["q_c_switcher_rrrr_3.csv", "q_c_switcher_rrcc_3.csv", "q_c_switcher_cccc_3.csv"]
    draw_trajectory(
